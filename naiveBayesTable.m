...
% Encode categorical variables (excluding Symbol and Name)
...
% Categorize Market Cap into predefined windows
...
% Add categorized Market Cap to the dataset
data.MarketCap = categorical(marketCapWindows);

% Convert remaining categorical variables to indices
data.Country = grp2idx(data.Country);
data.Sector = grp2idx(data.Sector);
data.Industry = grp2idx(data.Industry);
data.MarketCap = grp2idx(data.MarketCap); % Categorical Market Cap
data.Move = grp2idx(data.Move); % Target variable
...
target = data.Move; % Encoded Move variable as the target
...
% Construct the feature vector for the user input
...
for c = 1:numClasses
    ...
    for f = 1:numFeatures
        ...
        if any(idx)
            ...
            scores(c) = scores(c) * 0.0001; % Small smoothing factor for unseen values
        end
    end
end
